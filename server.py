...
def login():
    if request.method == "GET":
        ...
        if not check_password_hash(user.password, password):
            ...
            token = jwt.encode({'public_id': user.public_id, 'exp': datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(hours=24)}, app.config['SECRET_KEY'])
...
def register():
    if request.method == "GET":
        ...
        token = jwt.encode({'user_id': new_user.id, 'exp': datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=30)}, app.config['SECRET_KEY'], algorithm='HS256')
        send_mail(data['email'], token, type="register")
